...
class ReportYearRepositoryTest: ReportBasicTest() {

    private lateinit var repository: ReportYearRepository

    @BeforeEach
    fun setup() {
        repository = ReportYearRepository(reportClient)
    }
    ...
    fun `getData should return mapped report from client`() {
        ...
        val periodReport = PeriodReport(
            ...
            periodName = ReportDto.YEAR,
            income = income,
            expense = expense
        )

        whenever(reportClient.getThisYearReport(chatId.toString())).thenReturn(periodReport)

        val result = repository.getData(chatId)

        assertAll(
            { verify(reportClient).getThisYearReport(chatId.toString()) },
    ...
    fun `getData should return stub when client throws exception`() {
        whenever(reportClient.getThisYearReport(chatId.toString()))
