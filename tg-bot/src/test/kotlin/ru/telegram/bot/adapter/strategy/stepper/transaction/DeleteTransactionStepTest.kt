...
class DeleteTransactionStepTest {
    @Mock
    lateinit var errorService: ErrorService

    private lateinit var deleteTransactionStep: DeleteTransactionStep

    private val chatId = CHAT_ID
    private val expectedStep = StepCode.SHOW_TRANSACTIONS

    @BeforeEach
    fun setup() {
        deleteTransactionStep = DeleteTransactionStep(errorService)
    }

    @Test
    fun `getNextStep should delegate to errorService with SHOW_TRANSACTIONS as success step`() {
        whenever(errorService.resolveNextStep(chatId, expectedStep)).thenReturn(expectedStep)

        val result = deleteTransactionStep.getNextStep(chatId)
