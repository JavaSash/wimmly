...
class ReportTodayRepositoryTest: ReportBasicTest() {

    private lateinit var repository: ReportTodayRepository

    @BeforeEach
    fun setup() {
        repository = ReportTodayRepository(reportClient)
    }
    ...
    fun `getData should return mapped report from client`() {
        ...
        val periodReport = PeriodReport(
            ...
            periodName = ReportDto.TODAY,
            income = income,
            expense = expense
        )

        whenever(reportClient.getTodayReport(chatId.toString())).thenReturn(periodReport)

        val result = repository.getData(chatId)

        assertAll(
            { verify(reportClient).getTodayReport(chatId.toString()) },
    ...
    fun `getData should return stub when client throws exception`() {
        whenever(reportClient.getTodayReport(chatId.toString()))
