...
class EnterDateStepTest {
    @Mock
    lateinit var errorService: ErrorService

    private lateinit var enterDateStep: EnterDateStep

    private val chatId = CHAT_ID
    private val expectedStep = StepCode.ASK_COMMENT

    @BeforeEach
    fun setup() {
        enterDateStep = EnterDateStep(errorService)
    }

    @Test
    fun `getNextStep should delegate to errorService with ASK_COMMENT as success step`() {
        whenever(errorService.resolveNextStep(chatId, expectedStep)).thenReturn(expectedStep)

        val result = enterDateStep.getNextStep(chatId)
