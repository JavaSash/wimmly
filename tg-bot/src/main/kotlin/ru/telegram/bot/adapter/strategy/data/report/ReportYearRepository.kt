...
class ReportYearRepository(
    ...
    override fun getData(chatId: Long): ReportDto {
        logger.info { "$$$ Try to get year report for chat: $chatId" }
        return runCatching { mapToReportDto(reportClient.getThisYearReport(chatId.toString())) }
            .onFailure { logger.error { "$$$ Can't receive year report for user: $chatId. Cause: ${it.message}. Return stub" } }
